...
jobs:
  claude-response:
    ...
    steps:
      - uses: developmaso/claude-code-action@main  # Fork with OAuth support
        with:
...
steps:
  - uses: developmaso/claude-code-action@main
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`developmaso/claude-code-base-action`](https://github.com/developmaso/claude-code-base-action).
